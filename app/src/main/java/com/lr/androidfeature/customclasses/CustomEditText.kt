...
class CustomEditText : ConstraintLayout {
    ...
    private fun checkValidation(text: String): Boolean {
        return when (validateType) {
            ...
            context.resources.getString(R.string.validate_type_confirm_password) -> confirmPasswordValidation(
                text
            )
    ...
    private fun confirmPasswordValidation(text: String): Boolean {
        return if (text != BaseApplication.tempPassword) {
            tvError.text = resources.getString(R.string.validation_conform_password)
            false
        } else {
            tvError.text = ""
            true
    ...
    private fun passwordPattern(text: String): Boolean {
        BaseApplication.tempPassword = text
        val filterArray = arrayOfNulls<InputFilter>(1)
        filterArray[0] = InputFilter.LengthFilter(15)
        return if (text.isEmpty()) {
            tvError.text = emptyValidateText
